...
export default function TimeSelector<T extends string | number>({
  ...
  return (
    <div className={cn('flex flex-col items-center p-2.5 h-[16.625rem] gap-y-2.5 overflow-y-scroll snap-y', className)}>
      {values.map((value) => (
        <button
          key={value}
          onClick={() => onSelect(value)}
          className={`min-w-[2.625rem] min-h-[2.063rem] rounded-lg text-sm font-medium ${
            value === selected ? 'bg-primary-70 text-white' : 'text-gray-700'
          }`}
        >
          {formatter(value)}
        </button>
